package SQLPlugin.src.clientfacade;

import SQLPlugin.src.serverProxy.Poller;
import SQLPlugin.src.serverProxy.RealProxy;
import definitions.*;
...
public class Facade extends Observable {
	...
	private RobPlayerInfo[] _getPlayersOnHex(HexLocation hexLoc)
	{
		...
		List<RobPlayerInfo> robInfo = new ArrayList<RobPlayerInfo>();
		
		for(Integer i: indexes)
		{
			Player p = this.catanModel.playerManager.getPlayerByIndex(i);
			int playerIndex = p.getPlayerIndex();
			if(playerIndex == this._getLocalPlayerIndex())
			{
				continue;
			}
		...
		RobPlayerInfo[] robInfo2 = new RobPlayerInfo[robInfo.size()];
		for(int i = 0; i < robInfo.size(); i++)
		{
			robInfo2[i] = robInfo.get(i);
		}
		return robInfo2;
	}
     ...
     * @param randomPorts boolean representing whether the new game should have random Port shared.locations
